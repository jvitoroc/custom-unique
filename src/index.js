let customUnique = (function() {
    ...
    function getAllMatches(format, config) {
        ...
        var newFormat = format.replace(REGEX, function(match, key, times, index) {
            string = config[key];
            if (string == undefined || !config.hasOwnProperty(key))
                throw new Error("Some fields in format doesn't exist in config.");
            currentPos =
                prevMatch !== undefined ?
                prevMatch.pos + prevMatch.times + (index - beforeLastPos) :
                index;
            matches.push({
                key: key,
    ...
    function compile() {
        ...
        if (typeof format !== "string" || format.length < 1)
            throw new Error("The second argument (format) is invalid.");

        if (typeof config !== "object")
            throw new Error("The first argument (config) is invalid.");
